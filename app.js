...
app.get('/', (req, res) => {
  res.render('home', { homeContent: homeStartingContent, postdata: arr });
  // console.log( arr);
})
...
app.get('/post/:newPost', (req, res) => {
  ...
  arr.forEach((element) => {
    var newPosthanged1 = _.kebabCase(element.PostTitle);
    var newPosthanged = _.lowerFirst(newPosthanged1)
    if (newPosthanged === newPosthangedOld) {
      res.render('post', { pageTitle: element.PostTitle, pageContent: element.PostData });
    }
  })
})
